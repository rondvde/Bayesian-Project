...
model {
  ...
  // Priors
  sigma ~ gamma(29.9, 2);
  tau_depth ~ normal(0.9699 , 1);
  tau_sex ~ normal(2.9059, 1);
  
   for (j in 2:N_species) {
    gamma[j] ~ normal(20, 80);
    beta_width_2[j] ~ normal(tau_depth, 1);
    beta_sex_2[j] ~ normal(tau_sex, 1);
  }
  
  beta_width_2[1] ~ normal(2.9683, 1);
    beta_sex_2[1] ~ normal(0.1134, 1);
